...
function discoverSkills() {
	// OMP exposes skill routing in the prompt but no reliable complete active
	// catalog with exact bodies. Never treat routing text as a skill body, and
	// never silently report whole-stack coverage.
	const result = collectClass({
		id: "skills-active", kind: "skill", scope: "Active OMP model-invocable skill bodies (not exposed by the extension API)",
		members: [], expectedTotal: 1, complete: false,
		errors: [{ member_id: "skills-catalog", code: "SKILL_BODY_UNAVAILABLE", detail: "OMP extension context does not expose a complete active skill catalog with exact bodies; select skill bodies explicitly for an explicit-source audit" }],
		owner: "skill-providers", workflow: "skill-owner-review",
	});
	result.coverage.body_projection = "unavailable";
	result.coverage.projection_omissions = ["complete active skill catalog", "exact skill bodies"];
	return result;
}
...
export function createInstructionSourceDiscovery(config) {
	return async function discover(ctx, signal) {
		...
		const [system, agents, skills] = await Promise.all([
			discoverSystemPrompt(ctx), Promise.resolve(discoverInstructionFiles(config)), Promise.resolve(discoverSkills()),
		]);
		signal?.throwIfAborted();
		...
		return {
			...
			discovery: {
				schema: SOURCE_DISCOVERY_VERSION, mode: "automatic-current-session-partial-coverage",
				captured_at: new Date().toISOString(), provenance: "effective-current-omp-session-at-audit-time",
				agent_scoped: true, complete: classes.every(item => item.complete),
				classes, routes: [system.route, agents.route, skills.route],
				disclosure: "Included source members are retained in full locally and assessed as complete members or complete non-overlapping chunks. OMP does not expose complete skill bodies: automatic coverage is incomplete; use explicit source audits for selected skill bodies. Remote assessment requires separate typesafeEnabled and rsiInstructionDiscoveryEnabled opt-ins.",
			},
		};
	};
}
