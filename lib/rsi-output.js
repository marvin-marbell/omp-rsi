...
function auditInterpretation(value) {
	if (!value) return value;
	return { ...pick(value, ["disposition", "change_review_supported", "permission_effect", "policy_changed", "automatic_promotion", "automatic_apply", "next"]),
		counts: { findings: value.findings?.length ?? 0, uncertain: value.uncertain?.length ?? 0, outcome_supported_candidates: value.outcome_supported_candidates?.length ?? 0 },
...
export function rsiOutput(result) {
	...
	if (Array.isArray(result.stages)) return JSON.stringify({ ...common,
		...pick(result, ["status", "usage", "feedback", "publication", "improvement_cycle"]), coverage: coverage(result.coverage),
		capture: result.capture ? { capture_revision: result.capture.capture_revision, manifest: receipt(result.capture.manifest), snapshots_total: result.capture.snapshots?.length } : undefined,
		stages: result.stages.map(stage => ({ ...identity(stage), stage: stage.stage, units: stage.units })),
		synthesis: result.synthesis ? { status: result.synthesis.status, error_code: result.synthesis.error_code, interpretation: result.synthesis.interpretation, receipt: receipt(result.synthesis.receipt) } : undefined,
		interpretation: auditInterpretation(result.interpretation), proposal_guidance: result.proposal_guidance, receipt: receipt(result.receipt) });
