...
const FIELDS = {
	...
	review_observe: ["plan_id", "repository", "pr_number", "head_sha", "review_id", "review_commit", "review_state", "summary"],
	retrieval_observe: ["plan_id", "entry_path", "outcome", "summary"],
	corpus: ["kind", "after", "limit"],
	preview: ["kind", "source_id", "unit"],
	mine: ["kind", "source_ids", "after", "limit", "max_calls", "refresh", "cursor", "confirmations"],
...
export function createRsi(config, { memory, evaluate = evaluateTypesafe, signals, learningFactory = createLearning, discover }) {
	...
	async function run(action, fields, args = {}, exec = {}) {
		...
		if (action === "preview" || action === "mine" || action === "reduce") {
		...
		if (action === "review_observe") {
			const selected = fields;
			if (typeof selected.plan_id !== "string" || !selected.plan_id) throw new Error("review_observe requires a plan_id");
			if (typeof selected.repository !== "string" || !/^[A-Za-z0-9_.-]{1,100}\/[A-Za-z0-9_.-]{1,100}$/u.test(selected.repository)) throw new Error("Invalid repository identity");
			if (!Number.isSafeInteger(selected.pr_number) || selected.pr_number < 1) throw new Error("Invalid PR number");
			if (!Number.isSafeInteger(selected.review_id) || selected.review_id < 1) throw new Error("Invalid review ID");
			if (typeof selected.head_sha !== "string" || !/^[a-f0-9]{40}$/u.test(selected.head_sha)
				|| typeof selected.review_commit !== "string" || !/^[a-f0-9]{40}$/u.test(selected.review_commit)) throw new Error("PR and review commits must be full SHA-1 IDs");
			if (!["APPROVED", "CHANGES_REQUESTED", "COMMENTED", "DISMISSED"].includes(selected.review_state)) throw new Error("Invalid review state");
			if (typeof selected.summary !== "string" || !selected.summary.trim() || selected.summary.length > 500) throw new Error("Select a nonempty review summary of at most 500 characters");
			const context = await local({ action: "context", plan_ids: [selected.plan_id] }, args, exec);
			const data = { status: "observed", pr_review: { repository: selected.repository, number: selected.pr_number,
				head_sha: selected.head_sha, review_id: selected.review_id, commit_sha: selected.review_commit,
				state: selected.review_state, summary: selected.summary },
				provenance: "Caller-selected GitHub review fields, not authenticated against GitHub. Commit identity binds the reviewed head; changed heads invalidate approval.",
				disclosure: "Local selected fields only. An explicit preview then mine may send this review summary to TypeSafe.",
				disclaimer: DISCLAIMER };
			const result = await local({ action: "record", kind: "observation", bindings: bindingsFor(context), data }, args, exec);
			return { ...data, receipt: receipt(result), network_called: false };
		}
		if (action === "retrieval_observe") {
			if (typeof fields.plan_id !== "string" || !fields.plan_id) throw new Error("retrieval_observe requires a plan_id");
			if (!["useful", "stale", "contradicted", "missed", "unknown"].includes(fields.outcome)) throw new Error("Invalid retrieval outcome");
			if (typeof fields.summary !== "string" || !fields.summary.trim() || fields.summary.length > 500) throw new Error("Select a nonempty retrieval summary of at most 500 characters");
			const snapshot = await local({ action: "entry_snapshot", path: fields.entry_path }, args, exec);
			const context = await local({ action: "context", plan_ids: [fields.plan_id] }, args, exec);
			const data = { status: "observed", entry_retrieval: { path: snapshot.path, revision: snapshot.revision,
				bytes: snapshot.bytes, outcome: fields.outcome, summary: fields.summary },
				provenance: "The memory entry revision is hashed locally; retrieval outcome and summary are caller reports, not independently verified. No entry body is retained.",
				disclosure: "Local selected fields only. Preview the projection before an explicit TypeSafe mine.",
				disclaimer: DISCLAIMER };
			const result = await local({ action: "record", kind: "observation", bindings: bindingsFor(context), data }, args, exec);
			return { ...data, receipt: receipt(result), network_called: false };
		}
