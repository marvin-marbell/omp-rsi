...
const retryableDistributionSum = error => error instanceof TypesafeError
	&& error.code === "TYPESAFE_RESPONSE_INVALID" && error.validation_code === "TYPESAFE_INVALID_DISTRIBUTION_SUM";
function combinedUsage(left, right) {
	const inputTokens = left.input_tokens + right.input_tokens;
	const outputTokens = left.output_tokens + right.output_tokens;
	if (!Number.isSafeInteger(inputTokens) || !Number.isSafeInteger(outputTokens)) throw invalidResponse();
	return { input_tokens: inputTokens, output_tokens: outputTokens };
}

async function evaluateRequest(body, questions, config, apiKey, fetchImpl, op) {
	const retryUsage = { input_tokens: 0, output_tokens: 0 };
	let distributionSumRetries = 0;
	for (let attempt = 0; attempt <= config.typesafeRetries; attempt++) {
		...
		try {
			...
			payload = parseResponse(await readResponse(response, config.typesafeMaxResponseBytes, op));
			const result = validateResponse(payload, questions, apiKey);
			op.check();
			return { ...result, usage: combinedUsage(retryUsage, result.usage), attempts: attempt + 1, distributionSumRetries };
		} catch (error) {
			cancelBody(response);
			if (retryableDistributionSum(error) && distributionSumRetries < 1 && attempt < config.typesafeRetries) {
				// DISTRIBUTION_SUM is emitted only after the owned response envelope and
				// token usage passed validation. Count the paid failed attempt, but never
				// retain or normalize its provider probabilities.
				Object.assign(retryUsage, combinedUsage(retryUsage, payload.usage));
				distributionSumRetries++;
				await backoff(Math.min(MAX_BACKOFF_MS, 250 * 2 ** attempt), op);
				continue;
			}
			throw error;
		}
 ...
 * Returns { model, answers, usage, elapsedMs, attempts, distributionSumRetries };
 * probabilities stay raw and usage includes every paid distribution-sum retry.
...
export async function evaluateTypesafe(request, rawConfig = {}, { signal, apiKey, fetchImpl = globalThis.fetch } = {}) {
	...
	try {
		...
		return { model: result.model, answers: result.answers, usage: result.usage, elapsedMs: Math.max(0, performance.now() - started),
			attempts: result.attempts, distributionSumRetries: result.distributionSumRetries };
	} catch (error) {
