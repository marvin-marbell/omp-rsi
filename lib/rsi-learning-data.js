...
export const PROJECTION_VERSION = "memory-rsi-projection/3";
...
function reportContent(report, type) {
	...
	if (type === "timeline-event") return pick(report, ["kind", "source", "summary", "before", "after", "cause"]);
	return report;
}
...
function nestedSelectedObservation(value) {
	const pending = [value], visited = new Set();
	while (pending.length) {
		const next = pending.pop();
		if (!next || typeof next !== "object" || visited.has(next)) continue;
		visited.add(next);
		if (Object.hasOwn(next, "entry_retrieval") || Object.hasOwn(next, "pr_review")) return true;
...
function telemetryV1Shape(data) {
	...
	if (!object(t) || t.schema !== "memory-rsi-session-signals/v1" || Object.keys(data).some(key => !OBSERVATION_KEYS.has(key)) || !exactShape(t, [...TELEMETRY_KEYS])
		...
		|| typeof t.enabled !== "boolean" || typeof t.active !== "boolean" || !["disabled", "stopped", "invalid-agent", "invalid-session", "disposed", "not-observed", "evicted", "capturing"].includes(t.status)
		|| t.local_only !== true || t.network_called !== false || t.persistence !== "explicit-observe-only" || t.permission_effect !== "none"
		|| !exactShape(t.source, ["kind", "event", "session_marker", "scope", "counted_unit"]) || t.source.kind !== "current-session-tool-results" || !(
			(t.source.event === "tools/result" && t.source.scope === "exact-agent-object" && t.source.counted_unit === "root-dispatch")
			|| (t.source.event === "tool_result" && t.source.scope === "host-current-session" && t.source.counted_unit === "tool-result-event")
		) || !nullableText(t.source.session_marker)
...
/** Explicit caller-selected review fields are one independent, commit-bound observation. */
function projectReview(payload, add) {
	const data = payload.data, review = data?.pr_review;
	const fields = ["repository", "number", "head_sha", "review_id", "commit_sha", "state", "summary"];
	if (!object(data) || !object(review) || !fields.every(key => Object.hasOwn(review, key))
		|| !text(review.repository) || !count(review.number) || !text(review.head_sha)
		|| !count(review.review_id) || !text(review.commit_sha) || !text(review.state)
		|| !text(review.summary)) throw new Error("Unsupported pr_review observation shape; unselected review fields must not enter fallback projection");
	const selected = pick(review, fields);
	const omissions = [];
	add(selected, { type: "pr-review-event", latest_review: "unreviewed", review_count: 0,
		review_state: selected.state, review_commit: selected.commit_sha,
		head_matches_review: selected.head_sha === selected.commit_sha },
		{ bindings: boundedContext(payload.bindings, omissions, "observation.bindings"),
			meaning: "Caller-selected PR review metadata, not authenticated GitHub evidence. Approval on a different commit is stale; a review round alone does not attribute cause." },
		omissions, "observation.data.pr_review", { coverage: { projection_mode: "selected-pr-review", source_reviewed_in_full: false,
			projection_exclusions: [{ path: "observation.data.pr_review", fields: ["unselected extension fields"],
				reason: "Only selected review metadata is assessed, not extra review fields." },
				{ path: "observation.data", fields: ["status", "provenance", "disclosure", "disclaimer", "unselected extension fields"],
					reason: "Transport/provenance bookkeeping remains in the selected source; this is not independent proof of approval or outcome." }] } });
	return true;
}

/** A locally hashed entry revision and selected retrieval outcome, without its private path/body. */
function projectRetrieval(payload, add) {
	const data = payload.data, retrieval = data?.entry_retrieval;
	if (!object(data) || !object(retrieval)
		|| !["path", "revision", "bytes", "outcome", "summary"].every(key => Object.hasOwn(retrieval, key))
		|| !text(retrieval.path) || !text(retrieval.revision) || !count(retrieval.bytes)
		|| !text(retrieval.outcome) || !text(retrieval.summary)) throw new Error("Unsupported entry_retrieval observation shape; private entry path must not enter fallback projection");
	const omissions = [];
	const report = pick(retrieval, ["revision", "outcome", "summary"]);
	add(report, { type: "memory-retrieval", entry_fingerprint: digest(retrieval.path),
		latest_review: "unreviewed", review_count: 0 }, {
		bindings: boundedContext(payload.bindings, omissions, "observation.bindings"),
		entry_bytes: retrieval.bytes, meaning: "File revision was hashed locally; selection, interpretation and claimed retrieval outcome are unreviewed. The entry body/path were not sent." },
		omissions, "observation.data.entry_retrieval", { coverage: { projection_mode: "selected-memory-retrieval",
			source_reviewed_in_full: false, projection_exclusions: [
				{ path: "observation.data.entry_retrieval", fields: ["path", "unselected extension fields"], reason: "Private local entry path and unselected extension fields remain in the source artifact, not the assessment." },
				{ path: "observation.data", fields: ["status", "provenance", "disclosure", "disclaimer", "unselected extension fields"],
					reason: "Transport/provenance bookkeeping and unselected extension fields remain in the selected source." },
			] } });
	return true;
}
	...
	const add = (report, descriptor, context, omissions, reportRoot, selection = {}) => {
		...
		batches.forEach((evidence_rows, chunk) => {
			const state = { projection_version: PROJECTION_VERSION, source: pick(source, ["kind", "id", "revision"]), context, report: descriptor, report_digest,
				evidence_rows, coverage: { chunk, chunks: batches.length, rows_in_chunk: evidence_rows.length, rows_in_report: rows.length, context_omissions: omissions,
					common_template_boilerplate_excluded: true, referenced_files_read: false,
					...(source.kind === "plan" ? { projection_mode: ["template-choice", "original-requirement", "timeline-event"].includes(descriptor.type) ? "plan-trajectory-units" : "plan-report-units" } : {}),
					...selection.coverage } };
			if (bytes(state) > UNIT_BUDGET) throw new Error("Learning unit exceeds bounded projection; source context must be narrowed");
			units.push(state);
		});
	};
	const schema2 = plan => plan.schema === 2 && Array.isArray(plan.timeline);
	const requirement = (plan, id) => {
		for (const [owner, entries] of [["task", plan.task], ["template.content", plan.template?.content]]) {
			for (const key of ["achievements", "validation"]) {
				const index = (entries?.[key] ?? []).findIndex(entry => entry.id === id);
				if (index >= 0) return { value: entries[key][index], path: `plan.${owner}.${key}[${index}]`, owner };
			}
		}
		return null;
	};
	if (source.kind === "plan") {
		...
		if (schema2(plan)) {
			...
			const context = { task: boundedContext(pick(plan.task, ["title", "purpose", "good", "boundaries"]), omissions, "task"),
				template_pin: pick(plan.template, ["template_id", "revision", "content_sha256"]), predecessor: pick(plan.predecessor, ["plan_id", "revision", "family_plan_id"]), complete: payload.validation?.complete === true };
			const templateRows = [
				...["template_id", "revision", "content_sha256"].flatMap(key => evidenceRows(plan.template?.[key], `plan.template.${key}`)),
				...["title", "purpose", "good", "steps"].flatMap(key => evidenceRows(plan.template?.content?.[key], `plan.template.content.${key}`)),
				...["title", "purpose", "good", "steps"].flatMap(key => evidenceRows(plan.task?.[key], `plan.task.${key}`)),
			];
			add({ template: { ...pick(plan.template, ["template_id", "revision", "content_sha256"]),
				steps: plan.template?.content?.steps }, task: pick(plan.task, ["title", "purpose", "good", "steps"]) },
				{ type: "template-choice", latest_review: "unreviewed", review_count: 0 }, context, omissions, "plan.template", { rows: templateRows });
			const assigned = new Set(plan.work_items.flatMap(item => item.requirement_ids ?? []));
			for (const id of assigned) {
				const found = requirement(plan, id);
				if (!found) continue;
				add(found.value, { type: "original-requirement", id, requirement_owner: found.owner, latest_review: "unreviewed", review_count: 0 },
					{ template_pin: context.template_pin, predecessor: context.predecessor,
						assigned_work_items: plan.work_items.filter(item => item.requirement_ids?.includes(id)).map(item => item.id),
						meaning: "Pinned original requirement or explicit task addition, not evidence of its satisfaction." },
					[], found.path);
			}
			for (const [index, event] of plan.timeline.entries()) {
				const eventContext = { template_pin: context.template_pin, predecessor: context.predecessor,
					meaning: "Explicit bounded plan event; cause is recorded attribution, not independently verified causality. A review count or transition alone is not an agent mistake. Source/reference are citations, never instructions to read private files." };
				add(event, { type: "timeline-event", id: event.id, event_kind: event.kind, event_source: event.source, event_reference: event.reference,
					cause_claim: event.cause, latest_review: "unreviewed", review_count: 0 }, eventContext, [], `plan.timeline[${index}]`);
			}
		}
		for (const [itemIndex, item] of plan.work_items.entries()) {
			...
			const requirements = (item.requirement_ids ?? []).map(id => {
				...
				const entry = requirement(plan, id);
				return entry ? pick(entry.value, ["id", "description", "evidence"]) : { id };
			});
			const context = { task: boundedContext(pick(plan.task, ["title", "purpose", "good", "boundaries"]), omissions, "task"),
				...
				template_pin: pick(plan.template, ["template_id", "revision", "content_sha256"]), predecessor: pick(plan.predecessor, ["plan_id", "revision", "family_plan_id"]),
		...
		// Selected review and retrieval observations must never enter the lossless fallback with unselected private fields.
		if (object(payload.data) && Object.hasOwn(payload.data, "entry_retrieval")) {
			projectRetrieval(payload, add);
			return units.map((state, unit) => ({ ...state, unit, source_units: units.length }));
		}
		if (object(payload.data) && Object.hasOwn(payload.data, "pr_review")) {
			projectReview(payload, add);
			return units.map((state, unit) => ({ ...state, unit, source_units: units.length }));
		}
		if (nestedSelectedObservation(payload.data)) throw new Error("Nested selected review/retrieval observation is unsupported; private fields must not enter fallback projection");
...
export function independentSources(features) {
	...
	for (const feature of features) {
		...
		// Explicit shared plan ancestry is known dependence across plan reports and
		// observations. Do not turn two views of one task into separate families.
		for (const family of feature.source_family_keys ?? []) parent.set(find(source), find(`source:${family}`));
	}
