...
function projectReview(payload, add) {
	...
	if (!exactShape(data, ["status", "pr_review", "provenance", "disclosure", "disclaimer"])
		|| !exactShape(review, ["repository", "number", "head_sha", "review_id", "commit_sha", "state", "summary"])) return false;
	const omissions = [];
	add(review, { type: "pr-review-event", latest_review: "unreviewed", review_count: 0,
		review_state: review.state, review_commit: review.commit_sha,
		head_matches_review: review.head_sha === review.commit_sha },
		{ bindings: boundedContext(payload.bindings, omissions, "observation.bindings"),
		...
		omissions, "observation.data.pr_review", { coverage: { projection_mode: "selected-pr-review", source_reviewed_in_full: false,
			projection_exclusions: [{ path: "observation.data", fields: ["status", "provenance", "disclosure", "disclaimer"],
...
function projectRetrieval(payload, add) {
	...
	if (!exactShape(data, ["status", "entry_retrieval", "provenance", "disclosure", "disclaimer"])
		|| !exactShape(retrieval, ["path", "revision", "bytes", "outcome", "summary"])) return false;
	const omissions = [];
	...
	add(report, { type: "memory-retrieval", entry_fingerprint: digest(retrieval.path),
		...
		omissions, "observation.data.entry_retrieval", { coverage: { projection_mode: "selected-memory-retrieval",
			source_reviewed_in_full: false, projection_exclusions: [
				{ path: "observation.data.entry_retrieval.path", reason: "Private local entry path retained in the source artifact, not sent to the assessor." },
				{ path: "observation.data", fields: ["status", "provenance", "disclosure", "disclaimer"],
					reason: "Transport/provenance bookkeeping remains in the selected source." },
			] } });
	return true;
}
...
export function projectSource(source, payload) {
	...
	if (source.kind === "plan") {
		...
		if (projectReview(payload, add)) return units.map((state, unit) => ({ ...state, unit, source_units: units.length }));
		if (projectRetrieval(payload, add)) return units.map((state, unit) => ({ ...state, unit, source_units: units.length }));
		const omissions = [];
