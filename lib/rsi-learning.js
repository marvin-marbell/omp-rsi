...
export function createLearning({ local, assess, config = {} }) {
	...
	const sourceProjection = units => digest({ version: PROJECTION_VERSION, rubric: LEARNING_VERSION,
		requested_model: transport.typesafeModel, endpoint: transport.typesafeEndpoint,
		units: units.map(state => ({ state: digest(state), questions: learningIdentity(mapQuestions(state)).sha256 })) });
	const previewIdentity = (source, unit, projection) => digest({ source, unit, projection });
	...
	/** Exact selected remote input, locally projected without model or storage side effects. */
	async function preview(fields, args = {}, exec = {}) {
		validFields(fields, ["kind", "source_id", "unit"]);
		if (!["plans", "observations"].includes(fields.kind)) throw new Error("preview kind must be plans or observations");
		const id = ids([fields.source_id], 1, "source_id")[0];
		const unit = fields.unit ?? 0;
		if (!Number.isSafeInteger(unit) || unit < 0) throw new Error("unit must be a nonnegative integer");
		let payload, source;
		if (fields.kind === "plans") {
			const context = await local({ action: "context", plan_ids: [id] }, args, exec);
			payload = context.plans.find(plan => plan.plan_id === id);
			if (!payload) throw new Error("Selected plan is unavailable");
			source = { kind: "plan", id, revision: payload.revision };
		} else {
			payload = (await local({ action: "read", id }, args, exec)).artifact;
			...
			source = { kind: "observation", id, revision: payload.revision };
		}
		const units = projectSource(source, payload);
		if (unit >= units.length) throw new Error("unit is beyond selected source");
		const state = units[unit], questions = mapQuestions(state);
		const confirmation = randomUUID();
		const result = { source, unit, units_total: units.length, state, questions, confirmation,
			requested_model: transport.typesafeModel, endpoint: transport.typesafeEndpoint,
			request_bytes: bytes({ state, questions, model: transport.typesafeModel }),
			remote_enabled: config.typesafeEnabled === true, network_called: false,
			guidance: "Inspect this exact locally projected unit and pass its opaque confirmation in mine.confirmations for remote assessment. Each unit needs its own preview; a changed source revision or projection requires a new preview. The confirmation grants no permission or assurance about content or causality." };
		if (bytes(result) > DATA_BUDGET) throw new Error("Disclosure preview exceeds local output budget");
		previews.set(confirmation, previewIdentity(source, unit, sourceProjection(units)));
		if (previews.size > 512) previews.delete(previews.keys().next().value);
		return result;
	}

	async function mine(fields, args = {}, exec = {}) {
		validFields(fields, ["kind", "source_ids", "after", "limit", "max_calls", "refresh", "cursor", "confirmations"]);
		...
		if (!Array.isArray(confirmations) || confirmations.length > 512 || confirmations.some(token => typeof token !== "string" || !previews.has(token))
			|| new Set(confirmations).size !== confirmations.length) throw new Error("confirmations must be distinct opaque tokens from local previews in this runtime");
		...
		for (; sourceIndex < page.sources.length; sourceIndex++) {
			...
			const units = projectSource(source, payload);
			const projection = sourceProjection(units);
			...
			const coverage = { ...source, projection_version: PROJECTION_VERSION,
				...
				trajectory_units: { template_choices: units.filter(s => s.report.type === "template-choice").length,
					original_requirements: units.filter(s => s.report.type === "original-requirement").length,
					timeline_events: units.filter(s => s.report.type === "timeline-event").length },
			...
			for (let unit = start; unit < units.length; unit++) {
				...
				if (config.typesafeEnabled === true && !confirmations.some(token => previews.get(token) === previewIdentity(source, unit, projection))) {
					pause(source, unit, projection, "confirmation_required");
					result.warnings.push("Remote assessment requires the matching opaque local preview confirmation for this source revision, whole-source projection and unit. Preview this unit, then resume with its confirmation; no inference was performed for this unit.");
					return result;
				}
				result.calls++;
	...
	async function reduce(fields, args = {}, exec = {}) {
		...
		const warnings = ["Selected artifacts are not the whole growing corpus. Same tasks/revisions and normalized report copies are not independent evidence. Explicit plan events carry recorded attribution, not independently established causality; review rounds alone are not agent mistakes. Root lineage is checked before and after inference; the local store revalidates reachable source pins and hashes under cooperative writer locks at persistence. External edits that ignore those locks remain outside that transaction."];
