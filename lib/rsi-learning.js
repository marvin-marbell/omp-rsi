...
export function createLearning({ local, assess, config = {} }) {
	...
	async function preview(fields, args = {}, exec = {}) {
		...
		const result = { source, unit, units_total: units.length, state, questions,
			...
			guidance: "This exact projected state and typed questions would be disclosed by an explicit mine when remote TypeSafe is enabled. Inspect every unit and source revision first; a preview grants no permission and does not assert safe content or causality." };
