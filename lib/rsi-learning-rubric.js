...
export function mapFeature(artifact) {
	...
	let evidenceStatus = state.report.type === "planned" ? "planned" : unreviewed && a.evidence_status.choice === "observed" ? "unreviewed" : a.evidence_status.choice;
...
export function groupState(features, witnessLimit = 12) {
	...
	return { mechanism: features[0].mechanism, topic_excerpt: features[0].topic_excerpt,
		...
		meaning: "Counts describe typed reports, not votes for truth. Independent source count conservatively collapses same task/session, explicit shared plan ancestry and normalized report copies; it is not proven statistical independence. Omitted witnesses and unknown source relationships limit certainty." };
}
