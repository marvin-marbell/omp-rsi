...
export function mapFeature(artifact) {
	...
	const unreviewed = authorNote || ["timeline-event", "pr-review-event", "memory-retrieval"].includes(state.report.type) || (["evidence", "exception"].includes(state.report.type) && state.report.latest_review !== "accepted");
