...
/** Host-local defaults; DSH environment variables never select OMP storage. */
export const ompHome = () => resolve(join(homedir(), ".omp", "agent"));
export const defaultMemoryBase = () => join(ompHome(), "memory");

export function runtimePaths(config = {}) {
	const root = resolve(config.runtimeDir || join(ompHome(), "plugins", "omp-rsi", "runtime"));
