...
def _record(store, request, actor):
    shape(request, ("action", "kind", "bindings", "data"), ("record_id", "local_only"), label="record request")
    kind = request["kind"]
    local_only = request.get("local_only", False)
    if not isinstance(local_only, bool):
        raise ContractError("local_only must be boolean")
    if local_only and (kind != "audit" or not store.no_git):
        raise ContractError("local_only is reserved for no-git audit records")
    ...
    record_id = f"{'local-' if local_only else ''}{kind}-{uuid.uuid4().hex}"
    if "record_id" in request:
        if local_only:
            raise ContractError("local-only audit IDs are generated by the store")
...
def execute_request(request, base, *, actor="unknown", no_git=False, allow_non_main_branch=False):
    ...
    if paths:
        ...
        if store.local_protection is not None:
            result["persistence"]["local_only"] = store.local_protection
