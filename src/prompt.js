...
function guidance(config) {
	return [
		...
		config.rsiInstructionDiscoveryEnabled === true
			? "Automatic instruction audits are local-only and partial: OMP exposes the current effective system prompt and operator-configured instruction files, but not a complete active skill catalog with exact bodies. Missing skill coverage is reported, never inferred. Use explicit sources for selected skill bodies. Structural findings alone never authorize a policy change; require exact witnesses, reviewed cross-task outcomes and counterevidence. Never automatically apply or publish audit snapshots."
