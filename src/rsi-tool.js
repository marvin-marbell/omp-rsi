...
export function registerRsiTool(pi, config, runtime) {
	...
	pi.registerTool({
		...
		parameters: z.object({
			...
			request: z.string().optional().describe('JSON object excluding action. preview:{kind:\"plans\"|\"observations\",source_id,unit?} returns an opaque confirmation for the exact outbound unit locally without network; mine:{kind,source_ids? OR after?,limit?,max_calls?,refresh?,cursor?,confirmations?} assesses selected units only with matching preview.confirmation tokens when remote TypeSafe is enabled. audit:{sources?,plan_id?,feedback_ids?}; trial_spec:{trial_id,proposal_id,hypothesis,procedure,stopping_rule,environment,metrics,cases}; trial_results:{trial_id,trial_revision,results,review_note}; trial_review:{results_id}; preflight:{plan_id}; reflect:{plan_id,lesson?}; observe:{plan_id?,context_note?}; review_observe:{plan_id,repository,pr_number,head_sha,review_id,review_commit,review_state,summary}; retrieval_observe:{plan_id,entry_path,outcome,summary}; reduce:{artifact_ids,max_issues?}; prepare:{plan_ids,insight_ids?}; propose:{proposal_id,body OR edits,reason,expected_revision,plan_ids,source_artifact_ids?}; evaluate:{proposal_id}; read:{id,issue_index?}; list:{kind?,limit?,after?}; promote:{proposal_id,evaluation_id,expected_revision,review_note,apply?}.'),
