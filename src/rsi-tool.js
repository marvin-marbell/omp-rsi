...
export function registerRsiTool(pi, config, runtime) {
	...
	pi.registerTool({
		name: "memory_rsi",
		label: "Memory RSI",
		description: "Review memory policy and instruction sources with bounded, provenance-aware assessments. Actions: status, diagnose, audit, trial_spec, trial_results, trial_review, preflight, prepare, sections, propose, evaluate, reflect, observe, clear_signals, corpus, mine, reduce, read, list, promote. Remote assessment and automatic instruction discovery require separate operator opt-ins. Explicit-source audits remain available while automatic discovery is disabled; automatic coverage reports unavailable or unreadable skill bodies without inventing full-stack review. Proposals and promotion remain explicitly reviewed and revision-bound; nothing automatically applies.",
		parameters: z.object({
