...
`memory_rsi corpus` with `{"kind":"plans"}` discovers stored plan revisions without first naming a mistake. `memory_rsi preview` with `{"kind":"plans","source_id":"...","unit":0}` returns the exact projected state, typed questions and an opaque `confirmation` **locally**, without assessment or persistence. Inspect each selected unit, then pass its token in `memory_rsi mine` as `{"kind":"plans","source_ids":["..."],"confirmations":["<confirmation>", "..."]}`. A token is valid only in the current plugin runtime for that unit's source ID, revision and full projection (including questions, rubric, model and endpoint); changes require another preview. Preview all units you intend to assess, including units on later corpus pages or resumptions. Up to 512 previews are retained per runtime; older tokens expire. Uncached remote units lacking a matching confirmation pause with `confirmation_required` and a `resume` cursor rather than calling TypeSafe; cached successful mappings need no new assessment or confirmation. Explicit `mine` maps selected plan/template/requirement/event units; `reduce` groups linked hypotheses, independent task families, counterexamples and owner routes. `memory_rsi observe` captures current-session tool-result metadata on explicit request when telemetry is enabled. `memory_rsi review_observe` records only caller-selected `{plan_id,repository,pr_number,head_sha,review_id,review_commit,review_state,summary}` as a separate local observation; it does not fetch or authenticate GitHub reviews. `memory_rsi retrieval_observe` takes `{plan_id,entry_path,outcome,summary}`, hashes that ordinary selected entry's current bytes locally and stores its revision and reported retrieval outcome without the body. Retrieval projection transmits only selected revision, outcome, summary and bounded metadata; the private path and unselected extension fields stay local. Malformed retrieval shapes are rejected before assessment rather than entering the unknown-observation fallback. Local preview and matching confirmation are required before uncached opt-in remote `mine` for these observations. Caller-authored summaries themselves must not contain private paths or secrets. Event causes, review states and retrieval outcomes remain limited provenance, never proof of agent fault. Revised sources must be remapped; older mapping/insight artifacts use the prior rubric and cannot be reduced as current roots.

Remote TypeSafe requires both operator-enabled `typesafeEnabled` and an explicit `memory_rsi` assessment action. For `mine`, inspection and passing each preview's opaque confirmation are a local disclosure boundary, not an endorsement or a substitute for operator permission. With TypeSafe disabled, `mine` remains local even without confirmations; only an enabled, uncached assessment is blocked by missing or stale confirmation. Prompt preparation, plan creation and amendment make no remote requests; `preflight` is a separate explicit assessment. Selected event text is persisted in the operator's memory Git repository and follows its retention/sync policy: use bounded, nonsecret summaries and references, not raw transcripts or PR payloads. Tool-result telemetry remains metadata-only and needs its own opt-in; no hidden reasoning is available. PR feedback and user steering enter a plan only through explicit selected events; an external change is not an agent error.
